...
var (
	...
	cachePayThirdPaymentIdPrefix = "cache:pay:thirdPayment:id:"
	cachePayThirdPaymentSnPrefix = "cache:pay:thirdPayment:sn:"
)
...
type (
	...
	ThirdPayment struct {
		...
		OrderId        string    `db:"order_id"`         // 业务单号
...
func newThirdPaymentModel(conn sqlx.SqlConn, c cache.CacheConf, opts ...cache.Option) *defaultThirdPaymentModel {
...
func (m *defaultThirdPaymentModel) Delete(ctx context.Context, id int64) error {
	...
	payThirdPaymentIdKey := fmt.Sprintf("%s%v", cachePayThirdPaymentIdPrefix, id)
	payThirdPaymentSnKey := fmt.Sprintf("%s%v", cachePayThirdPaymentSnPrefix, data.Sn)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, payThirdPaymentIdKey, payThirdPaymentSnKey)
	return err
}
...
func (m *defaultThirdPaymentModel) FindOne(ctx context.Context, id int64) (*ThirdPayment, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, payThirdPaymentIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultThirdPaymentModel) FindOneBySn(ctx context.Context, sn string) (*ThirdPayment, error) {
	payThirdPaymentSnKey := fmt.Sprintf("%s%v", cachePayThirdPaymentSnPrefix, sn)
	var resp ThirdPayment
	err := m.QueryRowIndexCtx(ctx, &resp, payThirdPaymentSnKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultThirdPaymentModel) Insert(ctx context.Context, data *ThirdPayment) (sql.Result, error) {
	payThirdPaymentIdKey := fmt.Sprintf("%s%v", cachePayThirdPaymentIdPrefix, data.Id)
	payThirdPaymentSnKey := fmt.Sprintf("%s%v", cachePayThirdPaymentSnPrefix, data.Sn)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", m.table, thirdPaymentRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.Sn, data.DeleteTime, data.DelState, data.Version, data.UserId, data.PayMode, data.TradeType, data.TradeState, data.PayTotal, data.TransactionId, data.TradeStateDesc, data.OrderId, data.ServiceType, data.PayStatus, data.PayTime)
	}, payThirdPaymentIdKey, payThirdPaymentSnKey)
	return ret, err
}
...
func (m *defaultThirdPaymentModel) Update(ctx context.Context, newData *ThirdPayment) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, newData.Sn, newData.DeleteTime, newData.DelState, newData.Version, newData.UserId, newData.PayMode, newData.TradeType, newData.TradeState, newData.PayTotal, newData.TransactionId, newData.TradeStateDesc, newData.OrderId, newData.ServiceType, newData.PayStatus, newData.PayTime, newData.Id)
	}, payThirdPaymentIdKey, payThirdPaymentSnKey)
	return err
}

func (m *defaultThirdPaymentModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cachePayThirdPaymentIdPrefix, primary)
}

func (m *defaultThirdPaymentModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", thirdPaymentRows, m.table)
