...
type (
	...
	UserReceiveAddressModel interface {
		...
		ResetDefaultAddress(ctx context.Context, uid int64) error
	}
...
func (m customUserReceiveAddressModel) UpdateIsDelete(ctx context.Context, data *UserReceiveAddress) error {
	userReceiveAddressIdKey := fmt.Sprintf("%s%v", cacheUserUserReceiveAddressIdPrefix, data.Id)
...
func (m *customUserReceiveAddressModel) ResetDefaultAddress(ctx context.Context, uid int64) error {
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set is_default = 0 where `uid` = ?", m.table)
